...
async function installMockFloodguardApi(
  ...
  await page.route(`${apiBaseUrl}/api/**`, async (route) => {
    ...
    const pathAreaId = url.pathname.startsWith(signalPathPrefix)
      ? decodeURIComponent(url.pathname.slice(signalPathPrefix.length))
      : null;
    const areaId = pathAreaId || url.searchParams.get("area") || "parramatta";
