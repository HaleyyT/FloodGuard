...
const qualityFeatureFields = [
  ...
  "riverDelta3hM",
  "riverStationCount",
  "riverBaselineHeightM",
  "riverHeightDeltaM",
  "riverTendencyPressure",
  "riverLevelPressure",
  "rainfallPressure",
  "riverPressure",
  "wetnessPressure",
...
export function buildFeatureRows(historyRecords = []) {
  ...
  return chronological.map((record, index) => {
    ...
    return {
      ...
      rainfall1hMm: record.riskFeatures?.rainfall1hMm ?? 0,
      rainfall3hMm: record.riskFeatures?.rainfall3hMm ?? 0,
      rainfall24hMm: record.riskFeatures?.rainfall24hMm ?? 0,
      rainfall72hMm: record.riskFeatures?.rainfall72hMm ?? 0,
      antecedentWetnessMm: record.riskFeatures?.antecedentWetnessMm ?? 0,
      wetnessIndex: record.riskFeatures?.wetnessIndex ?? null,
      riverStationCount: record.river.stationCount ?? 0,
      riverPrimaryHeightM: record.river.primaryHeightM ?? null,
      riverLatestM: record.riskFeatures?.riverLatestM ?? record.river.primaryHeightM ?? null,
      riverDelta1hM: record.riskFeatures?.riverDelta1hM ?? null,
      riverDelta3hM: record.riskFeatures?.riverDelta3hM ?? null,
      riverTrend: record.riskFeatures?.riverTrend ?? "unknown",
      riverBaselineHeightM: record.riskFeatures?.riverBaselineHeightM ?? null,
      riverHeightDeltaM: record.riskFeatures?.riverHeightDeltaM ?? null,
      riverBaselineSampleCount: record.riskFeatures?.riverBaselineSampleCount ?? 0,
      riverBaselineMethod: record.riskFeatures?.riverBaselineMethod ?? "unknown",
      ...
      riverTendencyPressure: record.riskFeatures?.riverTendencyPressure ?? 0,
      riverLevelPressure: record.riskFeatures?.riverLevelPressure ?? 0,
      ...
      dataFreshnessScore: record.riskFeatures?.dataFreshnessScore ?? 0,
      freshnessScore: record.riskFeatures?.freshnessScore ?? null,
      inputCoverage: record.riskFeatures?.inputCoverage ?? record.riskSignals?.inputCoverage ?? 0,
      sourceCoverage: record.riskFeatures?.sourceCoverage ?? null,
      contributingSignalCount: record.riskFeatures?.contributingSignalCount ?? 0,
