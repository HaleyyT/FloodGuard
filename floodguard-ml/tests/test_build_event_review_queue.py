...
from build_event_review_queue import build_event_review_queue, write_review_queue_report  # noqa: E402
...
class BuildEventReviewQueueTests(unittest.TestCase):
    ...
    def test_build_event_review_queue_marks_placeholder_rows_as_missing_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            report_path = temp_path / "event_evidence_review_queue.md"
            ...
            write_review_queue_report(queue, output_path=report_path)
        ...
        self.assertEqual(written.loc[0, "can_become_reviewed_for_shadow_mode"], False)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            pd.DataFrame(
                [
                    {
                        "area": "parramatta",
                        "event_name": "Parramatta warning window",
            ...
            written = build_event_review_queue(backlog_path=backlog_path, output_path=output_path)

        self.assertEqual(len(written), 1)
        self.assertEqual(bool(written.loc[0, "evidence_is_real"]), True)
        self.assertEqual(written.loc[0, "area_match_status"], "high_confidence_area_match")
        self.assertEqual(written.loc[0, "time_window_status"], "window_present")
        self.assertEqual(bool(written.loc[0, "can_become_reviewed_for_shadow_mode"]), True)
